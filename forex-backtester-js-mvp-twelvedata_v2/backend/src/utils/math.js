...
export function equityStats(equityCurve) {
  if (!equityCurve || !equityCurve.length)
    return { start: 0, end: 0, maxDD: 0, returnPct: 0 };
...
function calcLotByRisk(equity, riskPercent, slPoints, valuePerPointPerLot) {
  if (!slPoints || slPoints <= 0) return 0;
  const riskMoney = equity * (riskPercent / 100);
  const lot = riskMoney / (slPoints * valuePerPointPerLot);
  // step 0.01 típico forex
